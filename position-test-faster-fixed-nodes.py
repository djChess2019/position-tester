...
def runTactics(epdFile, logFile, lc0_cmd, optString, weightPath, weight, nodeNum=None, moveTime=None):
    ...
    for line in epdfl:
        ...
        if move in best_moves:
            right += 1
            logLines.append(
                "; ".join(["1", str(move), bmstr, str(mnodes), idfield, weight, side, str(pieceNum), evalstr]))
            ...
            if mscore != None:
                ...
                mvstr += " no_score"
            logLines.append(
                "; ".join(["0", str(move), bmstr, str(mnodes), idfield, weight, side, str(pieceNum), evalstr]))
...
for weight in weights:  # loop over network weights, running problem set for each
    ...
    if nodes == None:
        ...
        outv = [weight, str(time), str(int(round(np.mean(nodesUsed)))), str(agreed), str(total),
                "%.3f" % ((100.0 * agreed) / total)]

    else:
        agreed, total, nodesUsed = runTactics(epdPath, logFile, lc0_cmd, optString, weightPath, weight, nodeNum=nodes)
        outv = [weight, str(nodes), str(int(round(np.mean(nodesUsed)))), str(agreed), str(total),
                "%.3f" % ((100.0 * agreed) / total)]
