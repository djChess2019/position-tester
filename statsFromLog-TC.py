...
class ChangeItem:
    def __init__(self, nodeCount, move, time, eval):
...
class PositionTesterLogItem:
    ...
    def __init__(self, line1: str, logNu: int):
        ...
        for x in range(self.changeCount):
            # nodeCount, move, eval
            self.changeList.append(ChangeItem(inputList[index], inputList[index + 1], inputList[index + 2], inputList[index + 3]))
            index += 4
...
def main():
    ...
    for setName in sets:
        ...
        agreeAtCount = 0
        while 2 * maxNodesUsed > checkAgreedAtNodes:
